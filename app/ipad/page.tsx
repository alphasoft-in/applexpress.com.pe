...
function IPadCard({ ipad, idx }: { ipad: any; idx: number }) {
  return (
    <motion.div
    ...
    >
      ...
      <div className="mb-4">
        <h3 className="text-2xl font-bold text-slate-900 dark:text-white">{ipad.model}</h3>
        {ipad.extra && <span className="inline-block mt-2 px-3 py-1 rounded-full bg-brand/10 text-brand text-xs font-semibold">{ipad.extra}</span>}
      </div>
      ...
      <div className="mt-auto pt-2 flex flex-col gap-3">
        ...
        <a 
          href={`https://wa.me/51982848503?text=Hola,%20me%20interesa%20el%20${encodeURIComponent(ipad.model)}`}
          target="_blank"
    ...
    <main className="min-h-screen bg-[#f5f5f7] dark:bg-black pt-32">
      ...
      <section className="py-20 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto">
        <div className="text-center mb-16">
          ...
          >
            Tu próxima computadora no es una computadora. Encuentra el iPad perfecto para ti.
          </motion.p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {IPADS.map((ipad, idx) => (
            <IPadCard key={idx} ipad={ipad} idx={idx} />
          ))}
        </div>
