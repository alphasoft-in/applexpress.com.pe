...
export async function generateStaticParams() {
  const { data: products } = await supabase.from("products").select("slug").eq("category", "ipad");
...
export async function generateMetadata(
  ...
  const specs = [item.processor, item.storage, item.screen].filter(Boolean).join(", ");
      ...
      <section className="pt-6 sm:pt-10 pb-16 sm:pb-20 px-4 sm:px-6 max-w-[980px] mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 md:gap-16 items-start">
          ...
          <StaggerContainer className="flex flex-col">
            ...
            <StaggerItem>
              <PurchaseOptions productName={item.model} productSpecs={[item.processor, item.storage, item.screen].filter(Boolean).join(" · ")} />
            </StaggerItem>
          </StaggerContainer>
        </div>
      </section>
