...
export function generateStaticParams() {
  return IPADS.map((item) => ({ slug: item.slug }));
}
...
export async function generateMetadata(
  ...
  const item = IPADS.find((i) => i.slug === slug);
  if (!item) return {};
  ...
  return {
    title,
    description,
    keywords: [`${item.model} Peru`, `${item.model} Lima`, `iPad ${item.chip} Peru`, "iPad importado Peru", "comprar iPad"],
    alternates: { canonical: `https://applexpress-com-pe.vercel.app/ipad/${slug}` },
    openGraph: {
      title: `${title} | Apple Express Perú`,
      description,
      url: `https://applexpress-com-pe.vercel.app/ipad/${slug}`,
...
export default async function IPadDetailPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const item = IPADS.find((i) => i.slug === resolvedParams.slug);
  if (!item) notFound();
