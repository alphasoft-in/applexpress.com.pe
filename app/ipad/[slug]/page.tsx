...
export async function generateStaticParams() {
  return IPADS.map((ipad) => ({
    slug: ipad.slug,
  }));
}

export default async function IPadDetailPage({ params }: { params: Promise<{ slug: string }> }) {
  const resolvedParams = await params;
  const ipad = IPADS.find((i) => i.slug === resolvedParams.slug);
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-12 items-start">
            ...
            <StaggerContainer className="flex flex-col justify-center">
                ...
                <ul className="space-y-3 text-sm">
                  ...
                  <li className="flex items-start">
                    ...
                    <div>
                      ...
                      <p className="text-slate-600 dark:text-slate-400">{ipad.screen}</p>
                    </div>
                  </li>
                  )}
                  {ipad.chip && (
                  <li className="flex items-start">
                    ...
                    <div>
                      ...
                      <p className="text-slate-600 dark:text-slate-400">{ipad.chip}</p>
                    </div>
                  </li>
                  )}
                  {ipad.storage && (
                  <li className="flex items-start">
                    ...
                    <div>
                      ...
                      <p className="text-slate-600 dark:text-slate-400">{ipad.storage}</p>
                    </div>
                  </li>
                  )}
                  {ipad.camera && (
                  <li className="flex items-start">
                    <CheckCircle2 className="w-5 h-5 mt-0.5 text-brand mr-3 flex-shrink-0" />
                    <div>
                      <p className="font-semibold text-slate-900 dark:text-white">Cámaras</p>
                      <p className="text-slate-600 dark:text-slate-400">{ipad.camera}</p>
                    </div>
                  </li>
                  )}
                  {ipad.battery && (
                  <li className="flex items-start">
                    ...
                    <div>
                      ...
                      <p className="text-slate-600 dark:text-slate-400">{ipad.battery}</p>
                    </div>
                  </li>
                  )}
                  {ipad.ports && (
                  <li className="flex items-start">
                    <CheckCircle2 className="w-5 h-5 mt-0.5 text-brand mr-3 flex-shrink-0" />
                    <div>
                      <p className="font-semibold text-slate-900 dark:text-white">Puertos</p>
                      <p className="text-slate-600 dark:text-slate-400">{ipad.ports}</p>
                    </div>
                  </li>
                  )}
                </ul>
              ...
              <StaggerItem className="flex flex-col gap-3">
                <a 
                  href={`https://wa.me/51934288165?text=Hola,%20quiero%20comprar%20el%20${encodeURIComponent(ipad.model)}`}
                  target="_blank"
