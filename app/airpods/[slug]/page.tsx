...
export function generateStaticParams() {
  return AIRPODS.map((item) => ({ slug: item.slug }));
}
...
export async function generateMetadata(
  ...
  const item = AIRPODS.find((i) => i.slug === slug);
  if (!item) return {};

  const specs = [(item as any).chip, (item as any).audio, item.battery].filter(Boolean).join(", ");
...
export default async function AirPodsDetailPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const item = AIRPODS.find((i) => i.slug === resolvedParams.slug);
  if (!item) notFound();
  ...
  const jsonLd = {
    ...
    "@type": "Product",
    name: item.model,
    description: `${item.model} importado desde EE.UU. ${(item as any).audio || ""}`,
    image: item.image,
      ...
      <section className="pt-6 sm:pt-10 pb-16 sm:pb-20 px-4 sm:px-6 max-w-[980px] mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 md:gap-16 items-start">
          ...
          <StaggerContainer className="flex flex-col">
            ...
            <StaggerItem>
              <PurchaseOptions productName={item.model} productSpecs={[(item as any).case, (item as any).extra].filter(Boolean).join(" · ")} />
            </StaggerItem>
          </StaggerContainer>
        </div>
      </section>
