...
function AirPodsCard({ airpods, idx }: { airpods: any; idx: number }) {
  return (
    <motion.div
    ...
    >
      ...
      <div className="mb-4">
        <h3 className="text-2xl font-bold text-slate-900 dark:text-white">{airpods.model}</h3>
        {airpods.extra && <span className="inline-block mt-2 px-3 py-1 rounded-full bg-brand/10 text-brand text-xs font-semibold">{airpods.extra}</span>}
      </div>
      ...
      <div className="mt-auto pt-2 flex flex-col gap-3">
        ...
        <a 
          href={`https://wa.me/51982848503?text=Hola,%20me%20interesan%20los%20${encodeURIComponent(airpods.model)}`}
          target="_blank"
    ...
    <main className="min-h-screen bg-[#f5f5f7] dark:bg-black pt-32">
      ...
      <section className="py-20 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto">
        <div className="text-center mb-16">
          ...
          >
            Magia para tus oídos. Descubre una experiencia de sonido envolvente sin cables.
          </motion.p>
        </div>

        {/* Product Cards Grid */}
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {AIRPODS.map((airpods, idx) => (
            <AirPodsCard key={idx} airpods={airpods} idx={idx} />
          ))}
        </div>
